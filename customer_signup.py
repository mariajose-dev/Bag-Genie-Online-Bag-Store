...
def phonenumber_exists(phone, exclude_id=None):
    """Check if the phone number already exists in the tbl_customer table."""
    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
            if exclude_id is None:
                sql = "SELECT COUNT(*) as count FROM tbl_customer WHERE Cust_phone = %s"
                cursor.execute(sql, (phone,))
            else:
                sql = "SELECT COUNT(*) as count FROM tbl_customer WHERE Cust_phone = %s AND Cust_id != %s"
...
def validate_user_input(data, current_phone=None, cust_id=None):
    ...
    if not data.get('phone'):
        ...
        if phonenumber_exists(data['phone'], exclude_id=cust_id):
    ...
    'district': r"^[A-Za-z\s]+$",
    'street': r"^[A-Za-z\s]+$",
    'security_answer': r"^[A-Za-z]+$"  # Only alphabetic characters for security answer
    }
...
def customer_signup_user(form_data):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            INSERT INTO tbl_customer (
                Username, Cust_fname, Cust_lname, Cust_city, Cust_dist,
                Cust_pin, Cust_street, Cust_phone, Cust_dob, Cust_gender,Favorite_Food,
                Cust_join, Cust_status
