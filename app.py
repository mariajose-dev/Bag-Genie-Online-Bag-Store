...
from staff_signup import staff_signup_user
...
def get_db_connection():
    return pymysql.connect(host="localhost", user="root", password="", database="bag_genie", cursorclass=pymysql.cursors.DictCursor)
...
def category_details(cat_id):
    ...
    return render_template('homecat.html', category=category, subcategories=subcategories,
                           user_data=user_data)
...
def subcategory_details(subcat_id):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            # Fetch items under subcategory, only those that have been purchased and have stock
            cursor.execute("""
    SELECT i.Item_id, i.Item_name, i.Item_image, pc.Sell_price,
           IFNULL(AVG(rr.Rating), 0) as avg_rating,
           COUNT(rr.Reviewrating_id) as review_count
    FROM tbl_item i
    LEFT JOIN (
        SELECT pc.Item_id, pc.Sell_price, pc.Expiry_date
    ...
    GROUP BY i.Item_id, i.Item_name, i.Item_image, pc.Sell_price
    ORDER BY i.Item_id
    """, (subcat_id,))
            ...
            # Ensure binary image data is converted properly
    ...
    return render_template('homesubcat.html', subcategory=subcategory, items=items, user_data=user_data)
...
def item_details(item_id):  
    ...
    try:
        with connection.cursor() as cursor:
            # Fetch item
            cursor.execute("""
                SELECT Item_name, Item_desc, Item_image, Sell_price 
                FROM tbl_item 
            ...
            avg_rating = float(avg_rating_result['avg_rating']) if avg_rating_result['avg_rating'] else 0.0
...
def add_to_cart(item_id):
    ...
    cursor.execute("""
        SELECT p.Sell_price, p.Stock
        FROM tbl_item i
        JOIN tbl_purchase_child p ON i.Item_id = p.Item_id
        WHERE i.Item_id = %s
    """, (item_id,))
    item = cursor.fetchone()
...
# Create payment record
    pay_date = datetime.now()  # Captures both date and time
    cursor.execute("""
...
"""
    ...
    if filters:
        ...
        if 'color' in filters and filters['color']:
            color_list = filters['color'].split(',')
            conditions.append(" OR ".join(["i.Item_name LIKE %s" for _ in color_list]))
            params += tuple(f"%{color}%" for color in color_list)
    ...
    # Convert Item_image to base64 for each item
    for item in items:
        if item and isinstance(item['Item_image'], bytes):
            item['Item_image'] = base64.b64encode(item['Item_image']).decode('utf-8')
...
def admindash():
    # Connect to the database
    conn = pymysql.connect(host='localhost', user='root', password='', database='bag_genie')
    cursor = conn.cursor()
    ...
    # Query to count the number of purchase
    ...
    return render_template('admindash.html', category_count=category_count,subcategory_count=subcategory_count,
...
def subcategory_management():
    ...
    print("Subcategories Data in Route:", subcategories, flush=True) 
    return render_template("subcategory_management.html", 
                           categories=categories,subcategories=subcategories,user_data=user_data)
...
def purchase_management():
    ...
    return render_template('purchase_management.html', 
                         vendors=vendors, 
                         items=items, 
                         purchases=purchases,edit_mode=False,user_data=user_data)
...
def staff_signup():
    ...
    if request.method == 'POST':
        ...
        flash('Staff Signup successful pending approval!', 'success')
        return redirect(url_for('loginpage'))  
...
def forgot_password():
    if request.method == "GET":
        return render_template("loginpage.html", show_forgot_password=True, username="")
    ...
    if step == "check_email":
        ...
        try:
            with connection.cursor() as cursor:
                ...
                return render_template("loginpage.html", show_forgot_password=True, 
                                     security_question_shown=True, username=username)
        finally:
            connection.close()
        ...
        try:
            with connection.cursor() as cursor:
                ...
                return render_template("loginpage.html", show_forgot_password=True, 
                                     password_reset_shown=True, username=username)
        finally:
            connection.close()
...
def customerdash():
    ...
    print("DEBUG - Customer ID:", cust_id)
    
    # Fetch orders
    query = """
        SELECT 
            cm.Cart_master_id, cm.Cart_item_status, cm.Cart_tot_amt,
            cc.Item_id, cc.Cart_qty, cc.Cart_unit_price, cc.Cart_price,
        ...
        WHERE cm.Cust_id = %s AND cm.Cart_item_status IN ('Paid', 'Out for Delivery', 'Delivered')
    ...
    if not orders_raw:
        ...
        for row in orders_raw:
            customer_orders = {}
        for row in orders_raw:
            row_dict = dict(row)  # Since rows are already dictionaries
        
            cart_master_id = row_dict['Cart_master_id']
            if cart_master_id not in customer_orders:
                customer_orders[cart_master_id] = {
                    'order_id': row_dict['Cart_master_id'],
                    'status': row_dict['Cart_item_status'],
                    'total_amount': row_dict['Cart_tot_amt'],
                    'max_date_arrival': row_dict['Max_date_arrival'],
                    'order_items': []
                }
            if row_dict['Item_id']:
                item_image = None
                if row_dict['Item_image'] is not None:
                    try:
                        item_image = base64.b64encode(row_dict['Item_image']).decode('utf-8')
                    except Exception as e:
                        print(f"DEBUG - Image Encoding Error: {e}")
                        item_image = None
                
                customer_orders[cart_master_id]['order_items'].append({
                    'item_id': row_dict['Item_id'],
                    'item_name': row_dict['Item_name'],
                    'item_image': item_image,
                    'quantity': row_dict['Cart_qty'],
                    'unit_price': row_dict['Cart_unit_price'],
                    'price': row_dict['Cart_price']
                })
        
        orders_list = list(customer_orders.values())
    
    cursor.close()
    db.close()
    return render_template('customerdash.html', orders=orders_list)
...
def search_items():
    ...
    try:
        with connection.cursor() as cursor:
            # Search items by name or description, only showing active items with stock
            cursor.execute("""
                SELECT DISTINCT i.Item_id, i.Item_name, i.Item_image
                FROM tbl_item i
                LEFT JOIN tbl_purchase_child pc ON i.Item_id = pc.Item_id
                WHERE (i.Item_name LIKE %s OR i.Item_desc LIKE %s)
                AND i.Item_status = 1
                AND pc.Stock > 0
                AND pc.Expiry_date > CURDATE()
                ORDER BY i.Item_name
                LIMIT 10
            """, (f"%{query}%", f"%{query}%"))  # Changed to search anywhere in both fields
            items = cursor.fetchall()
