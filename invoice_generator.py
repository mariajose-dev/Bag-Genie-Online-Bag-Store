...
def download_invoice(order_id, is_logged_in, get_db_connection):
    ...
    logo_path = os.path.join('static', 'images', 'bag genie new.jpg')
    if os.path.exists(logo_path):
        logo = Image(logo_path, width=0.5*inch, height=0.5*inch)
        elements.append(logo)
    else:
        elements.append(Paragraph("Bag Genie", styles['Heading2']))
...
<b>Time: </b>{pay_date.strftime('%I:%M:%S %p') if isinstance(pay_date, datetime) else ''}<br/>
    ...
    data = [
        ...
        [
            Paragraph("""
                <b>Company Name:</b> Bag Genie<br/>
                <b>Company Address:</b> Orient Square, Kadavanthra Junction, Kadavanthra, Kochi, Ernakulam, Kerala 682020<br/>
                Agrabah, AG 12345<br/>
                <b>Company Phone Number:</b> +91 9755447896<br/>
                <b>Company Email Address:</b> info@baggenie.com
            """, styles['Normal']),
            Paragraph(f"""
