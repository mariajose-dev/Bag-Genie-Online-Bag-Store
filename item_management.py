...
def validate_item(item_name, item_desc, item_profit):
    name_pattern = r"^[A-Za-z0-9][A-Za-z0-9\s-]{0,88}[A-Za-z0-9]$"
    # Simpler, more accurate pattern for your requirements
    desc_pattern = r"^[^\s][\s\S]{0,498}[^\s]$|^[^\s]$"
    
    if not re.match(name_pattern, item_name):
        return "Item name must be 1-90 characters long and can contain letters, numbers, spaces, and hyphens. No leading/trailing spaces."
    if not re.match(desc_pattern, item_desc):
        return "Item description must be 1-500 characters long and can contain letters, numbers, spaces, and punctuation. No leading/trailing spaces."
    
    # Check for newlines within the description (if you want to ensure it's well-formatted)
    lines = item_desc.split('\n')
    if len(lines) < 1 or len(lines) > 20:
        return "Description should contain between 1 and 20 lines."
