...
def validate_purchase_data(vendor_id, pur_date, item_ids, pur_qtys, pur_unit_prices, 
                          batch_nos, expiry_dates, item_doms):
    errors = []
    ...
    arrays = [item_ids, pur_qtys, pur_unit_prices, batch_nos, expiry_dates, item_doms]
...
def add_purchase():
    if request.method == "POST":
        ...
        errors = validate_purchase_data(vendor_id, pur_date, item_ids, pur_qtys, 
                                pur_unit_prices, batch_nos, expiry_dates,item_doms)
        if errors:
            for error in errors:
                flash(error, "danger")
        ...
        if not staff_id:
            flash("Staff authentication error ", "danger")
            return redirect(url_for('purchase_management'))
        ...
        try:
            ...
            for i in range(len(item_ids)):
                ...
                # Calculate item total and selling price
                ...
                # Insert purchase child record
                cursor.execute("""
                    INSERT INTO tbl_purchase_child 
                    (Pur_master_id, Item_id, Pur_qty, Pur_unit_price, Stock, 
                     Sell_price, Batch_no, Expiry_date, Item_dom)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (pur_master_id, item_id, pur_qty, pur_unit_price,pur_qty, 
                     sell_price, batch_nos[i], expiry_dates[i], item_doms[i]))

            # Update total amount in master record
            cursor.execute("""
                    ...
                    if pur_qty != old_qty or pur_unit_price != old_price:
                        ...
                        cursor.execute("""
                            ...
                            SET Item_id = %s, Pur_qty = %s, Pur_unit_price = %s, 
                                Stock = %s, Sell_price = %s, 
                                Batch_no = %s, Expiry_date = %s, Item_dom = %s
                            WHERE Pur_child_id = %s
                        """, (item_id, pur_qty, pur_unit_price,  pur_qty, 
                              sell_price, batch_nos[i], expiry_dates[i], item_doms[i], 
                              pur_child_id))
    ...
    try:
        ...
        cursor.execute("""
            SELECT pc.Pur_child_id, pc.Item_id, pc.Pur_qty, pc.Pur_unit_price, 
                    pc.Batch_no, pc.Expiry_date, pc.Item_dom, 
                   i.Item_name 
            FROM tbl_purchase_child pc
            JOIN tbl_item i ON pc.Item_id = i.Item_id
            WHERE pc.Pur_master_id = %s
