...
def get_all_couriers():  # Renamed from fetch_couriers to avoid naming conflict
    try:
        ...
        query = """
            SELECT 
                Courier_id, Username, C_cmpy_email, C_name, 
                C_city, C_dist, C_pin, C_street, C_phone, 
                C_join, Courier_status
            FROM tbl_courier 
...
'''
#company name exists
def courier_exists(c_name, exclude_id=None):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        if exclude_id is None:
            cursor.execute("SELECT COUNT(*) AS count FROM tbl_courier WHERE C_name = %s", (c_name,))
        else:
            cursor.execute("""
                SELECT COUNT(*) AS count 
                FROM tbl_courier
                WHERE C_name = %s AND Courier_id != %s
            """, (c_name, exclude_id))
            
        result = cursor.fetchone()
        return result['count'] > 0
    except Exception as e:
        print(f"Error checking company existence: {str(e)}")
        return False
    finally:
        connection.close()'''
...
def add_courier():
    if request.method == "POST":
        ...
        try:
            ...
            """
            cursor.execute(query, (
                staff_id,  # Staff_id from session
                data['username'],
                data['c_cmpy_email'],
                data['c_name'],
                data['c_city'],
